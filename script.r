df4 <- read.csv("usedcars.csv") # retrieve data from  Hard disk to RAM
str(df4)  #show the structure of usedcars.csv i.e. df3
df4 <- read.csv("usedcars.csv", stringsAsFactors = FALSE)
boxplot(df4$model)
dstr(df4)
yellow_cars <- subset(df4, color %in% "Yellow")  # make a subset of data from color which is yellow
high_mileage <-  subset(df4, mileage > 100000) # make a subset of data from mileage which ride > 10000
...
pie(table(df4$transmission)) # Draw pie chart
